...
\begin{document}
	...
	\begin{frame}
		...
		\p\begin{definition}[Morse-Bott functions]
			\p Let $M$ be a Riemannian manifold. A smooth submanifold $ N\subset M $ is said to be {\emph{\defn{non-degenerate critical submanifold}}} of $f$ if $N\subseteq \crf$ \p and for any $p\in N$, $\hess_{p}(f)$ is \alert<6>{non-degenerate in the direction normal to $N$ at $p$}. \p[7] The function $f$ is said to be {\emph{\defn{Morse-Bott}}} if the connected components of $ \crf $ are non-degenerate critical submanifolds.
		\end{definition}
